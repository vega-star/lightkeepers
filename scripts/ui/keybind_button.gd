...
func _input(event):
	if(do_set):
		if(event is InputEventKey):
			...
			InputMap.action_erase_event(keybind, key) #; print('Deleting {0} from {1}'.format({0: key, 1:keybind})) #? Delete the old input
			InputMap.action_add_event(keybind, event) #? Add the new key for this action
			text = OS.get_keycode_string(event.keycode) #? Return to the UI

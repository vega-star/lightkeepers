...
func load_tower(selected_tower : Tower) -> void:
	...
	if selected_tower != tower: #? Executes only if the new tower is different from the one actively stored
	...
	name_label.set_text(TranslationServer.tr(tower.tower_name.to_upper()))
	...
	_move(true) #? Show loaded and updated tower data

	...
	if draggable:
		...
		if Input.is_action_just_pressed('click'): # Click
			...
			'if source_object:
				...
				new_object.source_object = false
				new_object.volatile = true
				container.add_child(new_object)
				object = new_object
				source_quantity_change.emit(source_quantity, -1)'
...
func _return_pos(return_to_home : bool = false):
	...
	else: tween.tween_property(object, "global_position", initial_position, 0.2).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_EXPO)
